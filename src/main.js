...
const createFilmsListTopRatedContainerTemplate = () => {
  return (
...
// подозреваю, что это один контейнер, который добавляется два раза =_=

const createFilmsListMostCommentedContainerTemplate = () => {
  return (
    `<section class="films-list--extra">
...
render(filmsElement, createFilmsListTopRatedContainerTemplate());
render(filmsElement, createFilmsListMostCommentedContainerTemplate());

const filmsListExtraElements = mainElement.querySelectorAll(`.films-list--extra`);
...
for (let i = 0; i < EXTRA_FILM_COUNT; i++) {
  render(filmsTopRatedListElement, createFilmCardTemplate());
  render(filmsMostCommentedListElement, createFilmCardTemplate());
}
